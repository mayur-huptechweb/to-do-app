...
import { Link, useNavigate } from "react-router-dom";
import { toast } from "sonner";
import { login } from "@/api/auth"; // ✅ API wrapper
...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // data should look like: { token, user: {...} }
      loginUser(data.token); // ✅ save token to context + localStorage
        ...
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="flex flex-col space-y-2 mt-4">
            ...
            <p className="text-sm text-gray-500 text-center">
              ...
              <Link
                to="/signup"
                className="text-blue-600 hover:underline transition-all ease-in-out"
              >
                Sign up
              </Link>
            </p>
